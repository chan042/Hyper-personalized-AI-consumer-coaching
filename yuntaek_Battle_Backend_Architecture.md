...
- 윤택지수 대결은 사용자 2명이 월 단위로 1:1 대결을 만들고, 카테고리별 고정 미션 3개를 먼저 달성하며 추가 점수를 얻는 구조다.
- 실제 참가자 식별과 승패 계산의 원본은 `YuntaekBattle`, `BattleParticipant`, `BattleMission`, `BattleReward`이고, `BattleProfile.active_battle`, `pending_result_battle`는 진입 UX를 위한 캐시 포인터다.
- 진행 중 점수는 `mission_won_count`와 `mission_bonus_score`로만 보여주고, 월말에는 `MonthlyReport.score_snapshot`에서 카테고리 공식 점수를 읽어 최종 승패를 확정한다.
- 결과 조회 API는 조회 전용이며, `WAITING_FOR_SCORE` 상태에서는 `result_ready=false`만 내려준다. 실제 결과 확정은 배치가 담당한다.
- 챌린지 카테고리 미션은 `template_id`가 아니라 고정 `template_code`로 식별하고, legacy 데이터에 한해 `template_name` fallback을 둔다.
- 신청 만료는 운영 기준 `매월 16일 00:01 KST` 배치가 정리하고, 무지출 streak 정산은 매일 00:01 KST 배치가 처리한다.
...
- `BattleProfile.active_battle`, `pending_result_battle`는 캐시 포인터이므로, `GET /api/battles/entry/`, `GET /api/battles/profile/me/`, `POST /api/battles/profile/issue-code/`, `POST /api/battles/requests/` 같은 진입성 API 시작 전에 `reconcile_battle_profile(...)` 같은 경량 보정 함수를 한 번 태우는 편이 안전하다.
...
- 결과 조회 API(`GET /api/battles/{battle_id}/result/`)는 `score_snapshot` 생성이나 결과 확정을 직접 트리거하지 않고, 현재 상태만 읽는 read-only API로 두는 것이 안전하다.
- 즉 `WAITING_FOR_SCORE` 상태에서는 `result_ready=false` payload만 내려주고, 실제 결과 확정은 배치 또는 명시적 서버 작업에서만 수행하는 편이 토큰 중복 사용과 GET 부작용을 막을 수 있다.
...
- 미션이 `WON`으로 확정되면 양쪽 사용자에게 즉시 battle 알림을 만든다.
- 승자 본인에게는 `BATTLE_MISSION_WON`으로 `어떤 미션을 먼저 성공했는지`, `보너스 점수가 반영되었는지`를 알려준다.
- 상대에게는 `BATTLE_OPPONENT_MISSION_WON`으로 `누가 어떤 미션을 먼저 성공했는지`를 알려준다.
- 두 알림 모두 `related_id=battle_id`를 채우고, 클릭 시 `/challenge-battle/progress`로 이동시키는 편이 자연스럽다.
...
추가 원칙은 아래처럼 두는 편이 안전하다.

- 결과 조회 API는 `WAITING_FOR_SCORE` battle에 대해 `finalize_single_battle(...)`를 직접 호출하지 않는다.
- 즉 `GET /api/battles/{battle_id}/result/`는 상태를 바꾸지 않는 read-only 경로로 두고, 결과 확정은 배치(`finalize_waiting_battles`)만 수행하게 분리한다.
- 이렇게 해야 월간 리포트 생성과 battle 결과 확정이 사용자 GET 요청과 섞이지 않고, 같은 battle 결과 페이지 새로고침이 `ensure_score_snapshot(...)` 재시도를 유발하지 않는다.
...
- `BattleReward` row만 만들고 사용자 포인트를 실제로 올리지 않는 상태가 생기면 안 된다. 보상 지급의 원본은 `BattleReward + User.points/total_points_earned`가 함께 맞아야 한다.
- 따라서 보상 함수는 `BattleReward`를 `get_or_create(...)`한 뒤, 실제 row가 생성된 경우에만 `User.points`, `User.total_points_earned`를 함께 증가시키는 idempotent 구조로 두는 편이 안전하다.
...
| 순서 | 미션명 | 검증 방식 | 검증 규칙 |
|---|---|---|---|
| 1 | 3일 연속 무지출 챌린지 먼저 완료 | `challenge_template_complete` | 사전 등록된 `template_code='three_day_zero_spend'` 템플릿이고 `completed_at >= battle.started_at` |
| 2 | 3만원의 행복 먼저 완료 | `challenge_template_complete` | 사전 등록된 `template_code='thirty_thousand_happiness'` 템플릿이고 `completed_at >= battle.started_at` |
| 3 | 무00의 날 먼저 완료 | `challenge_template_complete` | 사전 등록된 `template_code='no_x_day'` 템플릿이고 `completed_at >= battle.started_at` |
...
- 챌린지 미션 식별은 변경 가능한 `template_id` 대신 고정 `template_code`를 기준으로 두는 편이 안전하다.
- 다만 하위 호환을 위해 legacy battle 스냅샷은 `template_name` fallback으로도 판정할 수 있게 두면 운영 데이터 복구에 유리하다.
...
무지출 streak 미션은 `성공 알림` 외에 `연속 기록 리셋 알림`도 함께 두는 편이 좋다.

- 배치가 전날을 닫으면서 확인한 결과, 목표 카테고리에 지출이 있었고 직전일까지 연속 기록이 1일 이상 이어지고 있었다면 해당 사용자에게만 `BATTLE_STREAK_RESET` 알림을 만든다.
- 이 알림은 미션 `실패` 확정이 아니라, 현재 연속 구간이 0으로 리셋되었고 오늘부터 다시 측정이 시작된다는 안내 성격이다.
- 메시지에는 `어느 날짜 지출 때문에`, `몇 일 연속 기록이 끊겼는지`, `언제 다시 측정이 시작되는지`를 함께 담는 편이 이해하기 쉽다.
- 같은 배치가 재실행되더라도 같은 사용자, 같은 battle, 같은 mission, 같은 기준 날짜에 대해 중복 알림이 쌓이지 않도록 dedupe 하는 편이 안전하다.
...
- 챌린지 미션 식별은 `template_code` 기준으로 고정하고, 필요하면 legacy 데이터에 한해 `template_name` fallback을 둔다.
...
- `GET /api/battles/profile/me/`
  ...
  - 응답 전 `reconcile_battle_profile(user)`를 수행해 `active_battle`, `pending_result_battle` 캐시 포인터를 보정하는 편이 안전하다.
- `POST /api/battles/profile/issue-code/`
  - 대결 코드 생성 또는 재발급
  - 코드 재발급 후에도 현재 대결 요약 응답을 함께 주려면 `reconcile_battle_profile(user)`를 다시 수행하는 편이 안전하다.
- `GET /api/battles/users/lookup/?battle_code=AB12CD34`
  - exact match 검색
...
- 이 GET API는 조회 전용으로 두고, 내부에서 `finalize_single_battle(...)`나 `ensure_score_snapshot(...)`를 직접 호출하지 않는 편이 안전하다.
...
- `d_day`는 `battle_end_at`가 아니라 `score_expected_at`의 날짜, 즉 결과 발표일(다음 달 1일) 기준 남은 일수로 계산하면 사용자가 이해하기 쉽다.
- 예를 들어 `score_expected_at=2026-04-01T00:00:00+09:00`이면 `2026-03-31`에는 `d_day=1`, `2026-03-30`에는 `d_day=2`가 된다.
...
미션 관련 battle 알림은 아래 3종을 추가한다.

- `BATTLE_MISSION_WON`
  - 대상: 미션을 먼저 성공한 사용자 본인
  - 의미: 특정 미션을 선점했고 `+3점`이 반영되었음을 안내
- `BATTLE_OPPONENT_MISSION_WON`
  - 대상: 상대 사용자
  - 의미: 상대가 어떤 미션을 먼저 성공했는지 안내
- `BATTLE_STREAK_RESET`
  - 대상: 무지출 streak가 끊긴 사용자 본인
  - 의미: 전날 목표 카테고리 지출로 인해 연속 기록이 끊겼고, 오늘부터 다시 측정이 시작됨을 안내

세 알림 모두 `notification_type=BATTLE`, `related_id=battle_id`를 사용하고, 클릭 시 `/challenge-battle/progress`로 이동시키는 편이 맞다.
...
- `BATTLE_REQUEST_RECEIVED`, `BATTLE_REQUEST_ACCEPTED`, `BATTLE_REQUEST_REJECTED`, `BATTLE_REQUEST_CANCELED`, `BATTLE_REQUEST_EXPIRED`, `BATTLE_MISSION_WON`, `BATTLE_OPPONENT_MISSION_WON`, `BATTLE_STREAK_RESET`, `BATTLE_WAITING_FOR_RESULT`
...
운영 기준 신청 만료 시각이 모두 `15일 23:59:59 KST`로 같다면, 아래 task는 `매월 16일 00:01 KST`에 1회 실행하는 편이 충분하다.
...
즉 현재 구조에서는 `5분/10분 주기`보다 `16일 00:01` 단발 배치가 정책과 더 잘 맞는다.
